...
(hint: you can always call `leaf.set(identity value)` or `myForest.set(identity, value)`.

Setter actions call `myleaf.set(identity, value)` and like that method, they can throw if invalid values are submitted.
...
```javascript
        const point = new Forest({
          $value: { q: 10, y: 20 }, actions: {
            double(leaf) {
              // @ts-ignore
              for (const [identity, value] of leaf.store.iter) {
                leaf.set(identity, 2 * value);
              }
            },
           addToEach(leaf, add) {
             for (const [identity, value] of leaf.store.iter) {
               leaf.set(identity, value + add);
             }
           },
...
Setting a root value with a identity that is mapped to a child will transmit the value downwards to that child. 
...
Children are attached to their parents by a reference on the child that has the child id and the identity it links to.
